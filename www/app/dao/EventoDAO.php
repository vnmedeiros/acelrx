...
class EventoDAO extends Base
{
    ...
    public function getDupla( $idEvento ) {
        $sql = "SELECT DISTINCT c1.nome as 'participante_1', c2.nome as 'participante_2', cat.descricao FROM duplas INNER JOIN cadastro c1 on duplas.participante_1 = c1.id INNER JOIN cadastro c2 on duplas.participante_2 = c2.id INNER JOIN cadastro_evento cad_ev ON cad_ev.id_cadastro = c1.id INNER JOIN categoria cat ON cad_ev.id_categoria = cat.id ORDER BY cat.id";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
